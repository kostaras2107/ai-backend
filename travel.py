...
ΚΑΝΟΝΕΣ για HOTEL_INTENT:
- Βάλε HOTEL_INTENT: true ΜΟΝΟ αν ο χρήστης ρωτάει για ΣΥΓΚΕΚΡΙΜΕΝΗ πόλη
  (π.χ. "αξιοθέατα στην Κωνσταντινούπολη", "που να φάω στη Ρώμη")
- Βάλε HOTEL_INTENT: false αν ο χρήστης ρωτάει γενικά για χώρα ή πολλές πόλεις
  (π.χ. "μέρη στην Τουρκία", "τι να δω στην Ιταλία")
- ΠΑΝΤΑ γράψε HOTEL_INTENT: true ή false στο τέλος

Παραδείγματα:
"αξιοθέατα στην Κωνσταντινούπολη" → HOTEL_INTENT: true (μία πόλη)
"μέρη στην Τουρκία" → HOTEL_INTENT: false (γενικό)
"που να φάω στη Ρώμη" → HOTEL_INTENT: true (μία πόλη)
"3 μέρες στην Ιταλία ποια μέρη" → HOTEL_INTENT: false (πολλές πόλεις)
"""
    completion = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[{"role": "system", "content": prompt}],
